/* ==========================================================
    ...
    Termination conditions:
        1. Hard cap:   N agents have reported max_cycles_reached
        2. Inactivity: N agents have reported idle_limit_reached
    ...
    Termination fires when either set contains N agents.
...
+!check_termination(N) <-
    ...
    ia.save_logs([idle_done(IdleCount), cycle_done(CycleCount), total(N), threshold(N)]);

    if (CycleCount >= N) {
        ia.save_logs([info("Termination: majority of agents reached max cycles.")]);
        .stopMAS
    } elif (IdleCount >= N) {
        ia.save_logs([info("Termination: majority of agents reached idle limit.")]);
        .stopMAS
    } else {
